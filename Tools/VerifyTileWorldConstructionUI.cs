// Construction regression on the TWC map; excludes unrelated fixed-width inventory layout checks.
using System;
using System.Linq;
...
public static class VerifyTileWorldConstructionUI
{
 ...
 public static async Task<string> Main()
 {
  ...
  Check(hud.Inventory.Category==BuildingCategory.Defense,"Default defense");Check(root.Q("inventory-items").childCount==s.TowerPrefabs.Length,"Registered defense prefabs");
  ...
  await Click("category-resource");Check(root.Q("inventory-items").childCount==0&&root.Q("inventory-empty").resolvedStyle.display!=DisplayStyle.None,"Empty category");
  ...
  return "PASS: category order/default/filter/empty/reopen, edit toggle cancels foundation/tower ghosts and reservations, edit-confirm enabled states, empty confirm, tower/foundation deferred click recovery, deselection/cancel, foundation with towers and empty foundation commit, removed auxiliary buttons, cleanup.";
 }
}
